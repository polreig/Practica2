...
grafico3<-ggplot(data=ds, mapping=aes(x=MinTemp,y=MaxTemp,color=RainTomorrow))+geom_point()+ggtitle("Relacion entre Mintemp y Maxtemp")
...
grafico4<-ggplot(data=ds,mapping=aes(x=Temp9am,y=Pressure9am,shape=RainToday,color=RainToday))+geom_point()+ggtitle("Relacion entre Temp9am y Pressure9am")
...
iris.grafico2rows<-ggplot(iris.tidy,aes(x=Species,y=Value,color=Part))+geom_point()+facet_grid(rows=vars(Measure))

iris.grafico2cols<-ggplot(iris.tidy,aes(x=Species,y=Value,color=Part))+geom_point()+facet_grid(cols=vars(Measure))
#en columnas se ve mejor

iris.grafico2jitter<-ggplot(iris.tidy,aes(x=Species,y=Value,color=Part))+geom_jitter()+facet_grid(cols=vars(Measure))
#ahora se puede ver como los puntos no se juntan en una misma linea, sino que estan dispersos y se observan mejor

print(iris.grafico2cols)
print(iris.grafico2jitter)
...
graficoEjer4<-ggplot(DatosEjer4,aes(x=Fecha,y=Ventas,color=Fabricante))+geom_point()+geom_line(aes(linetype=Fabricante,color=Fabricante))+ggtitle("Ventas mensuales de diferentes compañías tecnológicas")
...
grafico5.1<-ggplot(Experimento,aes(x=Dosis,y=Altura,color=Complemento,linetype=Complemento,shape=Complemento))+geom_point(size=3)+geom_line(size=1)+labs(title="Gráfico 1: Altura vs Dosis por Complemento",subtitle="Tipo de línea y forma de punto incluidos en aes()")

print(grafico5.1)

grafico5.2<-ggplot(Experimento,aes(x=Dosis,y=Altura,color=Complemento,linetype=Complemento,shape=Complemento))+geom_point(size=3)+geom_line(size=1)+geom_errorbar(aes(ymin=Altura-Error_Altura,ymax=Altura+Error_Altura),width=0.1)+labs(title="Gráfico 2: Altura vs Dosis con Barras de Error",subtitle="Incluye error estándar de la altura")

print(grafico5.2)

grafico5.3<-ggplot(Experimento,aes(x=Dosis,y=Altura,color=Complemento))+geom_point(aes(shape=Complemento),size=3)+geom_line(aes(linetype=Complemento),size=1)+geom_errorbar(aes(ymin=Altura-Error_Altura,ymax=Altura+Error_Altura),width=0.1)+scale_color_brewer(palette="Reds")+labs(title="Gráfico 3: Altura vs Dosis con Paleta de Rojos",subtitle="Tipo de línea y forma fuera del aes() principal")
...
graficoEjer6 <- ggplot(ice.river.df, aes(x = time))+geom_line(aes(y=flow.vat),color='red')+geom_line(aes(y=flow.jok),color='blue')+labs(
    title = "Caudal de Ríos Islandeses (1972-1974)",
    subtitle = "Comparación del caudal diario de Jokulsa (azul) y Vatnsdalsa (rojo)",
    x = "Fecha",
    y = "Caudal (m³/s)",
    caption = "Fuente: tseries::ice.river - Período: 1972-1974"
  )
...
load("data/weatherAUS_distributions.RData", verbose = TRUE)

grafico_boxplot<-ggplot(ds,aes(x=factor(Year),y=MinTemp))+geom_boxplot()+labs(title="Distribución de Temperaturas Mínimas por Año",x="Año",y="Temperatura Mínima (°C)")

print(grafico_boxplot)

grafico_violin<-ggplot(ds,aes(x=factor(Year),y=MaxTemp,fill=factor(Year)))+geom_violin()+labs(title="Distribución de temperaturas máximas por año",x="Año",y="Temperatura (ºC)")

print(grafico_violin)

grafico_final<-ggplot(ds,aes(x=factor(Year),y=MaxTemp,fill=factor(Year)))+geom_violin()+labs(title="Distribución de temperaturas máximas por año",x="Año",y="Temperatura (ºC)")+theme(legend.position="none",axis.text.x=element_text(angle=45,hjust=1))
